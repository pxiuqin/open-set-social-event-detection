...
class FunctionNegativeTripletSelector(TripletSelector):
    ...
    def get_triplets(self, embeddings, labels):
        ...
        distance_matrix = pdist(embeddings)  # 计算每对向量的欧式平方距离
        ...
        for label in set(labels):
            label_mask = (labels == label) # 找到labels中为label的条件，通过mask表示
            label_indices = np.where(label_mask)[0]  # 找到当前label索引
            if len(label_indices) < 2:
                continue
            negative_indices = np.where(np.logical_not(label_mask))[0]  # 非label的索引
            anchor_positives = list(combinations(label_indices, 2))  # All anchor-positive pairs  是一个组合，并不是以那个为anchor
            anchor_positives = np.array(anchor_positives)

            ap_distances = distance_matrix[anchor_positives[:, 0], anchor_positives[:, 1]]   # 通过distance_matrix找到距离值
...
def pairwise_sample(embeddings, labels=None, model=None):
    if model == None:#labels is not None:
        ...
        pair_labels = (labels[pairs[:,0]]==labels[pairs[:,1]])  # 相同的正、不同的为负
