...
def generate_english_features():
    ...
    np.save(save_path + 'features_69612_0709_spacy_lg_zero_multiclasses_filtered_English.npy', combined_features)
    print("Initial features saved.")

def generate_french_feature():

    # load dataset
    p_part1 = load_path + 'All_French.npy'
    df_np_part1 = np.load(p_part1, allow_pickle=True)
    df_np = df_np_part1
    print("Loaded data.")
    ...
    combined_features = np.concatenate((d_features, t_features), axis=1)
    print("Concatenated document features and time features.")
    np.save(save_path + 'features_69612_0709_spacy_lg_zero_multiclasses_filtered_French.npy', combined_features)
    print("Initial features saved.")

generate_french_feature()
